  ...
  try {
    ...
    res.status(201).json({
      ...
      user: {
        ...
        name: user.name,
        avatar: user.avatar,
        goals: user.wellnessProfile?.goals || [],
        fitnessLevel: user.wellnessProfile?.fitnessLevel || 'beginner',
        points: user.points,
        level: user.level,
        badges: user.badges
      }
...
router.get('/me', auth, async (req, res) => {
  try {
    res.json({
      ...
      goals: req.user.wellnessProfile?.goals || [],
      fitnessLevel: req.user.wellnessProfile?.fitnessLevel || 'beginner',
      points: req.user.points,
      level: req.user.level,
      badges: req.user.badges,
      streakFreeze: req.user.streakFreeze,
      preferences: req.user.wellnessProfile?.preferences || {},
